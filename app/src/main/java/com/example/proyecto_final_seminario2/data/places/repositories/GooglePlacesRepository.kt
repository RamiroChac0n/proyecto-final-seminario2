...
/**
 * Repositorio de busqueda de lugares.
 *
 * Intenta consultar Google Places y guarda resultados validos en Room. Si la
 * consulta remota falla, devuelve datos cacheados para mantener la experiencia
 * util sin conexion o sin API key.
 */
class GooglePlacesRepository(
    ...
    override suspend fun searchNearby(
        ...
        return remoteResult.fold(
            onSuccess = { remotePlaces ->
                // Google puede devolver elementos sin ID; esos no sirven para detalle/cache.
            ...
            onFailure = { error ->
                // Fallback por categoria cuando falla la busqueda cercana.
    ...
    override suspend fun searchByText(
        ...
        return remoteResult.fold(
            ...
            onFailure = { error ->
                // Fallback textual sobre la cache local cuando falla la busqueda remota.
    ...
    private suspend fun loadCachedPlaces(category: PlaceCategory) =
        // La categoria ALL recupera toda la cache; las demas filtran por etiqueta local.
